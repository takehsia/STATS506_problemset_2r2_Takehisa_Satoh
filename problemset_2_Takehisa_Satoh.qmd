...
hours_type <- c(
rep(1,8), #24,1,2,3,4,5,6,7
2,        #8
rep(3,8), #9,10,11,12,13,14,15,16
4,        #17  
rep(5,6)   #18,19,20,21,22,23
)
...
#category ii) can use a transformation prior to being used as the outcomeor iii) #category iii)not be appropriate
...
```{r}
#add log transformation of "view_count", "like_count", "comment_count"
...
# c
# answer:
# -The variable "danger" has a positive effect on "like_count" at 10% significant level.
# -The variable "patriotic" has a positive effect on "dislike_count" at 10% significant level.
# -The variable "patriotic" has a positive effect on "comment_count" at 10% significant level.
...
predictors <- paste(c(flags,"year"),collapse ="+")

lm_results <- list()

for (y in outcomes){
formula_str<- paste(y,"~",predictors)
model <- lm(as.formula(formula_str), data = youtube)
lm_results[[y]] <- summary(model)
cat("\n------ Linear regression result for", y, "-------\n")
print(summary(model))
}
```
